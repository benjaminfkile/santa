...
| `LatestMessage` overlay | `snapshot.event.latestMessage` | Collapsible ticker; `aria-live="polite"` |
...
Data row units: speed as mph from `speedMps`, heading as degrees plus cardinal from `headingDeg`, altitude as feet from `altitudeM`, accuracy as feet from `accuracyM`; a null field shows a placeholder from `copy.ts`. `recordedAt` and `receivedAt` are shown in the data row as times; they decide nothing. Sections after the `map` section on the live page render below the fold; the page scrolls.
...
- `variant: "panel"` (collapsible, top five visible, expand for all) as the live screen's overlay; `variant: "full"` wherever a `leaderboard` section asks for it, the ended page in the starter content. `data.emptyText` renders when there are no types.
...
`GET /me/cookies` also runs when the sheet reopens; the site keeps no cookie state between opens beyond what the last response said.
...
On mount: `GET /me` and `GET /me/subscriptions` in parallel. Renders:
...
- While `eventStatusId` is 3 every path renders the live page except the four site-coded paths (`/auth/callback`, `/alerts/verify`, `/alerts/unsubscribe`, `/preview`); the other pages come back when the status changes. No link out of the live screen during the event.
