...
- `personSignIn.ts`: clicks the visible sign-in control (the header action on wide viewports; the panel entry, opened through the Menu button, on narrow viewports and on the map page), fills the E2E person's email and password on the site's own `/auth/sign-in` page, submits, and waits for the signed-in control the same way; a second helper `personSignUp.ts` drives sign-up and confirmation for a throwaway `+tag` address whose code the harness reads through `AdminGetUser`-free means: the dev pool's test person is pre-confirmed, so the sign-up spec asserts only the "We emailed you a code" step and then deletes the unconfirmed user with `AdminDeleteUser` through the harness's AWS credentials.
