...
export const Countdown: SectionComponent = ({ data, bundle }) => {
  ...
  return (
    <div className={styles.countdown} data-testid="countdown">
      {heading ? (
        <p className={styles.countdownHeading}>
      ...
      {timeReady && remaining !== null ? (
        ...
        <div className={styles.countdownBlank} aria-hidden />
      )}
...
function CountdownDigits({ remaining }: { remaining: number }) {
  ...
  return (
    <div className={styles.countdownDigits} aria-label={text}>
      {parts.map((part, i) => {
        const value = part.replace(/[a-z]+$/i, "");
        return (
          <span key={i} className={styles.countdownCell}>
